...
function Form(props) {
  ...
  const addNote = e => {
    ...
    if (existingDate.length > 0) {
      setTable(prevTable=>(
        prevTable.map((note) =>{
          if(note.date === form.date) {
            return {date: note.date, distance: String(Number(note.distance) + Number(form.distance)), action: note.action}
          } else return note
       } 
       ) 
       ))
      ...
      setTable(prevTable => ([...prevTable, {
        'date': form.date, 'distance': form.distance, 'action': <svg width="15px" height="15px" enableBackground="new 0 0 415.188 415.188" version="1.1" viewBox="0 0 415.19 415.19" xmlSpace="preserve" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
